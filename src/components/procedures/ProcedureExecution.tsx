    ...
    return <div className="flex flex-col h-full">
        ...
        <div className="flex-1 overflow-y-auto p-6">
          <div className="max-w-4xl mx-auto space-y-4">
            {formattedAnswers.length > 0 ? formattedAnswers.map((answer, index) => <Card key={index} className="border-l-4 border-l-blue-500">
        ...
        <div className="border-t bg-white p-6">
          <div className="flex justify-between items-center max-w-4xl mx-auto">
            ...
            <div className="flex gap-3">
              ...
              <Button onClick={completeExecution} className="bg-green-600 hover:bg-green-700 flex items-center gap-2" disabled={submitExecution.isPending}>
  ...
  return <div className="flex flex-col h-full">
      ...
      <div className="flex-1 overflow-y-auto p-6 bg-gray-50">
        <div className="max-w-2xl mx-auto">
          {currentField && <Card className="shadow-sm">
              <CardContent className="p-8">
                <ExecutionFieldRenderer field={currentField} value={answers[currentField.id || `field_${currentStep}`] || ''} onChange={value => handleAnswerChange(currentField.id || `field_${currentStep}`, value)} error={errors[currentField.id || `field_${currentStep}`]} fieldId={currentField.id || `field_${currentStep}`} />
              </CardContent>
            </Card>}
        </div>
      </div>
      ...
      <div className="border-t bg-white p-6">
        <div className="flex justify-between items-center max-w-4xl mx-auto">
          ...
          <div className="flex gap-2">
            {fields.map((_, index) => <div key={index} className={`w-3 h-3 rounded-full transition-all ${answers[fields[index].id || `field_${index}`] ? 'bg-green-500' : index === currentStep ? 'bg-blue-500' : 'bg-gray-300'}`} />)}
          </div>
          
          <Button onClick={goToNextStep} className="bg-blue-600 hover:bg-blue-700 flex items-center gap-2">
