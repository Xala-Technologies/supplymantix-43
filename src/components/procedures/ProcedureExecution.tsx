...
export const ProcedureExecution: React.FC<ProcedureExecutionProps> = ({
  ...
  return <div className="flex flex-col h-full">
      ...
      <div className="flex-1 overflow-y-auto p-4">
        {currentField && <ExecutionFieldRenderer field={currentField} value={answers[`field_${currentStep}`] || ''} onChange={value => handleAnswerChange(`field_${currentStep}`, value)} error={errors[`field_${currentStep}`]} fieldId={`field_${currentStep}`} />}
      </div>
      ...
      {/* Progress Summary */}
      {Object.keys(answers).length > 0 && <div className="flex-shrink-0 border-t bg-gray-50 p-2">
          <div className="text-center">
            ...
            <div className="flex justify-center space-x-1">
              {fields.map((_, index) => <div key={index} className={`w-2 h-2 rounded-full ${answers[`field_${index}`] ? 'bg-green-500' : index === currentStep ? 'bg-blue-500' : 'bg-gray-300'}`} />)}
            </div>
          </div>
        </div>}
    </div>;
