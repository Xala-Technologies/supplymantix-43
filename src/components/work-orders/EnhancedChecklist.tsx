...
import { useChecklistEnhanced } from "@/hooks/useChecklistEnhanced";

interface EnhancedChecklistProps {
  workOrderId: string;
}

export const EnhancedChecklist = ({ workOrderId }: EnhancedChecklistProps) => {
  ...
  const {
    ...
    isRemoving,
    isToggling,
  } = useChecklistEnhanced(workOrderId);
  ...
  const handleToggleComplete = async (id: string, completed: boolean) => {
    try {
      await toggleComplete({ itemId: id, completed });
    } catch (error) {
      ...
      <CardContent>
        ...
        <div className="space-y-3">
          {checklistItems.map((item) => (
            <div key={item.id} className="flex items-start gap-3 p-3 border rounded-lg group hover:bg-gray-50">
