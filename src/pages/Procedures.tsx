...
const Procedures = () => {
  ...
  const { data: proceduresData, isLoading, error, refetch } = useProceduresEnhanced({
  ...
  // Process the procedures data safely
  const procedures = React.useMemo(() => {
    ...
    return proceduresData.procedures.map((proc: any) => ({
      ...proc,
      // Ensure fields property exists and is properly formatted
      fields: proc.procedure_fields || proc.fields || [],
      // Map procedure_fields to fields for consistency
      ...(proc.procedure_fields && { fields: proc.procedure_fields })
    }));
  }, [proceduresData]);
